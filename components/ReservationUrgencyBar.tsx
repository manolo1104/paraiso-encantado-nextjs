...
export default function ReservationUrgencyBar() {
  ...
  useEffect(() => {
    ...
    const countdown = setInterval(() => {
      setTimeLeft((prev) => {
        if (prev <= 1) {
          if (!timerFired.current) { timerFired.current = true; trackEvent('TIMER_EXPIRED'); }
    ...
    return () => { clearInterval(viewInterval); clearInterval(bookingInterval); clearInterval(countdown); };
  }, []);
  ...
  return (
    <>
      <style>{`
        ...
        .urgency-item svg { flex-shrink: 0; }
        .urgency-dot {
      ...
      <div className="urgency-bar" role="status" aria-live="polite">
        <div className="urgency-inner">
          ...
          <div className="urgency-item">
            <Zap size={13} strokeWidth={2} color="#c9a97a" aria-hidden="true" />
            <span>Solo {scarcity} suites disponibles en {currentMonth}</span>
          </div>
          <span className="urgency-divider" aria-hidden="true">·</span>
          <div className="urgency-item">
            <Check size={13} strokeWidth={2.5} color="#22c55e" aria-hidden="true" />
            <span>
          ...
          {timeLeft > 0 && (
            <>
              <span className="urgency-divider" aria-hidden="true">·</span>
              <div className="urgency-item">
                <Clock size={13} strokeWidth={1.5} color="#c9a97a" aria-hidden="true" />
                <span>
                  Precio garantizado:&nbsp;
