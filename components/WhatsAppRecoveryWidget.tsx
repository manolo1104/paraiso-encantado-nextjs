...
export default function WhatsAppRecoveryWidget({ bookingData = {} }: Props) {
  ...
  // Timers + exit intent — run once on mount
  useEffect(() => {
    // 3 min after BOOKING_START without completing
  ...
  const handleWhatsAppClick = () => {
    const parts: string[] = ['Hola, estaba reservando en Paraíso Encantado'];
    ...
    parts.push('y tengo una pregunta.');
  ...
  return (
    <>
      ...
      <div className="wa-popup" role="dialog" aria-modal="true" aria-labelledby="wa-popup-title">
        ...
        <h3 className="wa-title" id="wa-popup-title">¿Necesitas ayuda?</h3>
        <p className="wa-desc">
          Estoy aquí para responder tus preguntas y ayudarte a completar tu reserva.
        </p>
        ...
        <button className="wa-no" onClick={handleClose}>No gracias</button>
      </div>
    </>
  );
}
