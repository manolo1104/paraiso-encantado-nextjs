...
export async function POST(req: NextRequest) {
  try {
    ...
    } else if (event === 'ir_formulario') {
      console.log(`🛒 CHECKOUT  checkin=${payload.checkin ?? '-'}  checkout=${payload.checkout ?? '-'}  huéspedes=${payload.guests ?? '-'}  src=${payload.source ?? '-'}  ip=${ip}`);
    } else if (event === 'salir_pagina') {
      console.log(`🚪 SALIDA  tiempo=${payload.secondsOnPage}s  sid=${sessionId?.slice(0, 12)}`);
    } else {
